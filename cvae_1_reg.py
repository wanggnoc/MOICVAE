...
if __name__ == "__main__":
    ...
    scaler = MinMaxScaler()
    R = pd.read_csv(path,header=None,sep='\t')
    ...
    if args.rating:  # feed in with rating
        data_set = np.zeros((np.shape(Rtensor)[0] * np.shape(Rtensor)[1], 3))#N*3的指示矩阵
        count = 0
        for i in range(np.shape(Rtensor)[0]):
            for j in range(np.shape(Rtensor)[1]):                
                data_set[count][0] = i
                data_set[count][1] = j
                data_set[count][2] = Rtensor[i,j]
                count += 1
        ...
        for train_index, test_index in kf.split(data_set):#索引值，label
            ...
            for ele in DTItrain:
                Xtrain[int(ele[0]), int(ele[1])] = ele[2]#矩阵形式
                data_train_index[int(ele[0]),int(ele[1])] = 1
            for ele in DTItest:
                Xtest[int(ele[0]), int(ele[1])] = ele[2]#矩阵形式
            print(data_set[:,2])
...
##############@@@wd##############
            ...
            pred_list_label = []
            for ele in DTItrain:
                pred_list.append(Zscore[int(ele[0]), int(ele[1])])
                pred_list_label.append(1 if Zscore[int(ele[0]), int(ele[1])]>0.5 else 0)
            ...
            for ele in DTItest: #预测的元素
                ...
                pred_list_label.append(1 if Zscore[int(ele[0]), int(ele[1])]>0.5 else 0)
                ground_truth.append(ele[2])
                y_ic50.append(Zscore[int(ele[0]), int(ele[1])])
                y_hat_ic50.append(ele[2])
                index_list.append((ele[0], ele[1]))

        np.savetxt('./y_ic50_GDSC_2.txt',y_ic50)
        np.savetxt('./y_true_ic50_GDSC_2.txt',y_hat_ic50)
        np.savetxt('./index.txt',index_list)
