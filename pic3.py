import numpy as np
from sklearn.metrics import confusion_matrix, accuracy_score,f1_score,roc_auc_score,recall_score,precision_score,precision_recall_curve
...
def rocs_plot(P2, Y2,setname=['']):
    ...
    for pp, yy,col in zip(Y2,P2,colors):
        precision, recall, thresholds = precision_recall_curve(pp,  yy)
        pr_auc = metrics.auc(recall, precision)   
        pyplot.plot(recall,precision, lw=1, color=col, linewidth=1.5, alpha=0.9, label="%s:AP=%0.3f" % (setname[i],pr_auc))
        i += 1
    pyplot.xlim([0.00, 1.0])
    pyplot.ylim([0.00, 1.0])
    pyplot.xlabel("Recall",fontsize=12)
    pyplot.ylabel("Precision",fontsize=12)
    pyplot.title("Precision-Recall curve of GDSC ",fontsize=14,fontweight='bold')
    pyplot.legend(loc="lower right")
    pyplot.savefig('PR.png',dpi=200)
