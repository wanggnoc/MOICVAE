...
if __name__ == "__main__":
    ...
    parser.add_argument('--threshold', help='determine threshold for test', type=float, default=0.0)
    ...
    if args.rating:  # feed in with rating
        ...
        kf = StratifiedKFold( n_splits=10, shuffle=True, random_state=23)#23
        y_ic50 = []
        y_hat_ic50 = []
        for train_index, test_index in kf.split(data_set[:, range(2)],data_set[:, 2]):#索引值，label
            ...
            print(len(Rtensor[1,:]),len(Rtensor2[1,:]))
...
##############@@@wd##############
            ...
            for ele in DTItest: #预测的元素
                pred_list.append(Zscore[ele[0], ele[1]])
                pred_list_label.append(1 if Zscore[ele[0], ele[1]]>0.68 else 0)
                ground_truth.append(ele[2])
...
#         np.savetxt('./y_ic50.txt',y_ic50)
#         np.savetxt('./y_hat_ic50.txt',y_hat_ic50)
    else:  # feed in with side information
##################################################################
        path2 = 'drugmdaFeatures3.txt'
        X2 = np.loadtxt(path2)
        #X2 = X2.transpose()
        #X2 = fea2.transpose()
        #X2[X2 > 0] = 1
